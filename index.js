...
class KnexDataSource extends DataSource {
  ...
  createLog(cacheString) {
    return (res) => {
      if (DEBUG) console.timeEnd(cacheString);
      return res;
    }
  }
  ...
  execute(query, ttl = 0) {
    ...
    if (DEBUG) console.time(cacheString);
    const log = this.createLog(cacheString)
    const cacheKey = this.getCacheKey(cacheString);
    let promise = this.memoizedResults.get(cacheKey);
    if (promise) return promise.then(log);
    if (!ttl) {
